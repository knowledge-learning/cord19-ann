...
class Model:
    ...
    def train_similarity(self):
        ...
        for i, sentence in enumerate(tqdm.tqdm(self.corpus)):
            doc = nlp(sentence.text)
            docs.append(TaggedDocument([token.text for token in doc], [i]))

        self.doc2vec = Doc2Vec(docs, min_count=1, epochs=100, vector_size=32)
...
class ClassifierEntity:
    ...
    def relation_features(
        ...
        if len(doc_from) == 0 or len(doc_to) == 0:
            logger.debug("Cannot align tokens to relation %r" % relation)
            return None
    ...
    def train_relations(self, collection: Collection):
        ...
        for i, sentence in enumerate(tqdm.tqdm(collection.sentences)):
            ...
            for relation in sentence.relations:
                features = self.relation_features(relation, doc=doc)
